...
SMOOTH_ALPHA = 0.4  # pour lissage curseur
...
_scroll_frames = 0
_scrolling = None
_smooth_x, _smooth_y = None, None  # lissage curseur
...
def detect_hand_gesture(frame):
    ...
    global _scroll_frames, _scrolling, _smooth_x, _smooth_y
    ...
    if _is_index_only(pts):
        ...
        # Limitation verticale (évite scroll accidentel)
        if y_norm > 0.85:
            y_norm = 0.85
        ...
        if _smooth_x is None:
            _smooth_x, _smooth_y = screen_x, screen_y
        else:
            _smooth_x = int(SMOOTH_ALPHA * screen_x + (1 - SMOOTH_ALPHA) * _smooth_x)
            _smooth_y = int(SMOOTH_ALPHA * screen_y + (1 - SMOOTH_ALPHA) * _smooth_y)

        pyautogui.moveTo(_smooth_x, _smooth_y, duration=0.01)
        return "index_mouse"
...
def reset_state():
    global _prev_wrist, _accum_x, _last_pose, _hold_frames, _scroll_frames, _scrolling, _smooth_x, _smooth_y
    _prev_wrist = None
    _accum_x = 0
    _last_pose = None
    _hold_frames = 0
